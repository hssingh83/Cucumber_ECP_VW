...
//	Assert.assertTrue(driver.findElements(hammersTab).isEmpty());
	...
	public void location1() {
		...
		int l=driver.findElement(scriptsTab).getLocation().getX();
		int M= driver.findElement(scriptsTab).getLocation().getY();
		
		System.out.print("value of xlocation: "+l);
		System.out.println("value of ylocation: "+M);
...
public void tagTab() {
		...
		if (driver.findElement(tagTab).isDisplayed()) {
			...
			log.info("Verified that 'Tags Tab' is appearing on the VoiceWatch dashboard");
			
						
		}else
		
		{
			log.info("Verified that 'Tags Tab' is not appeared on the VoiceWatch dashboard");
